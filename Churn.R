...
plot(telecom$Churn, col="green", main="Bar Plot of levels of the customer churning within the telecom data set", xlab="Is churn happening ?",ylab="No. of customers")

# Logistic Regression using 70:30 partition 
...
Cpre <- predict(LR,test, type='response')
summary(Cpre)

Cpre<-ifelse(Cpre>0.5,1,0)
table(test$Churn,Cpre)

str(test$Churn)
confusionMatrix(test$Churn,Cpre)
misclas<-mean(test$Churn!=Cpre)
misclas
Accuracy <- 1-misclas
Accuracy

testing$Churn <- as.character(test$Churn)
testing$Churn[testing$Churn=="No"] <- "0"
testing$Churn[testing$Churn=="Yes"] <- "1"
fitted.results <- predict(LR,newdata=testing,type='response')
